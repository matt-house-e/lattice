...
Key parameters: `model`, `temperature`, `system_prompt`, `system_prompt_header`, `client`, `base_url`, `max_retries`, `cache`, `structured_outputs`, `depends_on`.

### `FunctionStep(name, fn, fields, ...)`

Wraps any sync or async callable. Receives `StepContext`, returns `dict[str, Any]`.

Key parameters: `cache`, `cache_version`, `depends_on`.
